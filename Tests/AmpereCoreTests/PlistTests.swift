...
/// fields; `InstallCommands` (ampere-cli target) is the one that writes it.
@Suite struct PlistTests {
    ...
    @Test func programArgumentsIsAnArrayContainingOnlyTheBinaryPath() {
        let xml = launchdPlist(label: "com.ampere.daemon", binaryPath: "/tmp/ampered")
        #expect(xml.contains("<key>ProgramArguments</key>"))
        #expect(xml.contains("<array>"))
        #expect(xml.contains("<string>/tmp/ampered</string>"))
    }
    ...
    @Test func passesPlutilLint() throws {
        ...
        let tempURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("ampere-plist-test-\(UUID().uuidString).plist")
