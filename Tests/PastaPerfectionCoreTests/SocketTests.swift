...
/// capped at ~104 bytes), so tests use `/tmp/pastaperfection-test-<pid>-<n>.sock`
...
@Suite(.serialized) struct SocketTests {
    ...
    /// Short unix socket path (max ~104 chars): `/tmp/pastaperfection-test-<pid>-<n>.sock`.
    private static func makeSocketPath(_ n: Int) -> String {
        "/tmp/pastaperfection-test-\(ProcessInfo.processInfo.processIdentifier)-\(n).sock"
    }
    ...
    // target, `PastaPerfectionCoreTests`, has no visibility into `pastaperfectiond`'s
