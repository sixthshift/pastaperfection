...
/// login item only makes sense when Ampere is running from inside a proper
/// app bundle (`dist/Ampere.app`, installed via `scripts/make-app.sh`) — a
/// bare `.build/debug/Ampere` binary has no bundle identifier and
...
public enum LoginItem {
    ...
    /// `.build/(debug|release)/Ampere` directly.
    ...
    /// Registers Ampere as a login item. No-op (returns `false`) when
    ...
    /// Unregisters Ampere as a login item. No-op (returns `false`) when
