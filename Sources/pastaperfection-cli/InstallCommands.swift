...
public enum InstallCommands {
    ...
    static func runInstall(_ args: [String], dryRun: Bool) -> Int32 {
        ...
        do {
            ...
            guard status == 0 else {
                writeStderr("pastaperfection-cli: '\(bootstrapCommand)' failed (exit \(status))\n")
                return 1
            }
            ...
            writeStderr("pastaperfection-cli: install failed: \(error)\n")
            return 1
        }
    }
    ...
    /// Default source binary: `pastaperfectiond` living next to the running
    /// `pastaperfection-cli` binary (same directory as `CommandLine.arguments[0]`).
    static func defaultSiblingBinaryPath() -> String {
        ...
        return (base as NSString).appendingPathComponent("pastaperfectiond")
    }
    ...
    static func runState() -> Int32 {
        ...
        do {
            ...
            writeStderr("pastaperfection-cli: \(error)\n")
            return 1
        }
        defer { client.close() }
        ...
        do {
            ...
            guard decoded.ok, let data = decoded.data else {
                writeStderr("pastaperfection-cli: get-state failed: \(decoded.error ?? "unknown error")\n")
                return 1
            }
            ...
            writeStderr("pastaperfection-cli: get-state request failed: \(error)\n")
            return 1
        }
    }
    ...
    static func runReq(_ args: [String]) -> Int32 {
        guard let line = args.first else {
            writeStderr("pastaperfection-cli: usage: pastaperfection-cli req <json-line>\n")
            return 64
        }
        ...
        do {
            ...
            writeStderr("pastaperfection-cli: req failed: \(error)\n")
            return 1
        }
    }
    ...
    static func runProcess(_ path: String, _ arguments: [String]) -> Int32 {
        ...
        do {
            ...
            writeStderr("pastaperfection-cli: failed to run \(path): \(error)\n")
            return 1
        }
    }
